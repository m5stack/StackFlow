...
private:
    ...
    std::unique_ptr<sherpa_onnx::AxclModel> axera_session_;
...
#define OPTS_AUTO_SET(obj, key)              \
    ...
    int load_model_axera(const nlohmann::json &config_body)
    {
        ...
        try {
            ...
            axera_session_ = std::make_unique<sherpa_onnx::AxclModel>(model_file.c_str());
            if (!axera_session_->IsInitialized()) {
...
#undef OPTS_AUTO_SET
    ...
    std::vector<float> run_inference(const std::vector<float> &audio_chunk_16k)
    {
        ...
        axera_session_->SetInputTensorData("input", mat_flattened.data(), mat_flattened.size());
        axera_session_->SetInputTensorData("cache", axera_cache_.data(), axera_cache_.size());
        bool ret = axera_session_->Run();
        if (!ret) {
            SLOGE("axera_session run failed!");
            return {};
        }
        std::vector<float> out_chunk = axera_session_->GetOutputTensorData("output");
        std::vector<float> r_cache   = axera_session_->GetOutputTensorData("r_cache");
        if (r_cache.size() == axera_cache_.size()) {
            std::memcpy(axera_cache_.data(), r_cache.data(), axera_cache_.size() * sizeof(float));
        }
        return out_chunk;
    }
    ...
    bool delete_model()
    {
        ...
        if (axera_session_) axera_session_.reset();
