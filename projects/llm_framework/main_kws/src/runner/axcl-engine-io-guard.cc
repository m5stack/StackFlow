...
AxclEngineIOGuard::AxclEngineIOGuard(axclrtEngineIOInfo io_info)
{
    ...
    if (ret != 0) {
        SHERPA_ONNX_LOGE("Failed to call axclrtEngineCreateIO(). Return code is: %d", static_cast<int32_t>(ret));
        SHERPA_ONNX_EXIT(-1);
    }
...
AxclEngineIOGuard::~AxclEngineIOGuard()
{
    if (initialized_) {
        ...
        if (ret != 0) {
            SHERPA_ONNX_LOGE("Failed to call axclrtEngineDestroyIO(). Return code is: %d", static_cast<int32_t>(ret));
            SHERPA_ONNX_EXIT(-1);
        }
    }
}
