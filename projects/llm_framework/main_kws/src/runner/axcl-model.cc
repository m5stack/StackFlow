...
#include "axcl-engine-guard.h"
#include "axcl-engine-io-guard.h"
#include "axcl-engine-io-info-guard.h"
#include "axcl-manager.h"
#include "utils.h"
#include "macros.h"
...
public:
    ...
    Impl(const void *cpu_buf, size_t buf_len_in_bytes, int32_t device_id)
    {
...

        {
            ...
            auto ret = axclrtMemcpy(device_ptr, cpu_buf, buf_len_in_bytes, AXCL_MEMCPY_HOST_TO_DEVICE);
            if (ret != 0) {
                SHERPA_ONNX_LOGE("Failed to call axclrtMemcpy(). Return code is: %d", static_cast<int32_t>(ret));
                return;
            }

            ret = axclrtEngineLoadFromMem(device_ptr, buf_len_in_bytes, &model_id_);
            if (ret != 0) {
                SHERPA_ONNX_LOGE("Failed to call axclrtEngineLoadFromMem(). Return code is: %d",
    ...
    ~Impl()
    {
        if (model_loaded_) {
            ...
            if (ret != 0) {
                SHERPA_ONNX_LOGE("Failed to call axclrtEngineUnload(). Return code is: %d", static_cast<int32_t>(ret));
                SHERPA_ONNX_EXIT(-1);
            }
        }
    }
    ...
    template <typename T>
    bool SetInputTensorData(const std::string &name, const T *p, int32_t n) const
    {
        EnsureThreadContext();
        for (size_t i = 0; i < input_tensor_names_.size(); ++i) {
            if (input_tensor_names_[i] == name) {
                if (n * sizeof(T) != input_tensors_[i].Size()) {
                    SHERPA_ONNX_LOGE("Expected size: %zu, given: %zu", input_tensors_[i].Size(), n * sizeof(T));
                    return false;
                }

                auto ret =
                    axclrtMemcpy(input_tensors_[i].Get(), p, input_tensors_[i].Size(), AXCL_MEMCPY_HOST_TO_DEVICE);
                if (ret != 0) {
                    SHERPA_ONNX_LOGE(
    ...
    bool SetInputTensorDataRaw(const std::string &name, const void *p, int32_t nbytes) const
    {
        EnsureThreadContext();
        for (size_t i = 0; i < input_tensor_names_.size(); ++i) {
            if (input_tensor_names_[i] == name) {
                if (static_cast<size_t>(nbytes) != input_tensors_[i].Size()) {
                    SHERPA_ONNX_LOGE("Expected size: %zu, given: %d", input_tensors_[i].Size(), nbytes);
                    return false;
                }
                auto ret =
                    axclrtMemcpy(input_tensors_[i].Get(), p, input_tensors_[i].Size(), AXCL_MEMCPY_HOST_TO_DEVICE);
    ...
    std::vector<float> GetOutputTensorData(const std::string &name) const
    {
        ...
        for (size_t i = 0; i < output_tensor_names_.size(); ++i) {
            if (output_tensor_names_[i] == name) {
                ...
                auto ret = axclrtMemcpy(out.data(), output_tensors_[i].Get(), bytes, AXCL_MEMCPY_DEVICE_TO_HOST);
                if (ret != 0) {
                    SHERPA_ONNX_LOGE(
                        "Failed to call axclrtMemcpy(). tensor name: '%s', return code: "
                        "%d",
    ...
    bool Run() const
    {
        EnsureThreadContext();
        uint32_t group = 0;
        auto ret       = axclrtEngineExecute(model_id_, context_id_, group, *engine_io_guard_);
        if (ret != 0) {
            SHERPA_ONNX_LOGE("Failed to call axclrtEngineExecute(), return code: %d", static_cast<int32_t>(ret));
            return false;
        }
        return true;
    }
...
private:
    bool SetDevice(int32_t device_index)
    {
        ...
        if (ret != 0) {
            SHERPA_ONNX_LOGE("Failed to call axclrtGetDeviceList(). Return code is: %d", static_cast<int32_t>(ret));
            return false;
        }
        ...
        if (device_index < 0 || device_index >= lst.num) {
            SHERPA_ONNX_LOGE("Invalid device_index: %d. Valid range: 0-%d", device_index, lst.num - 1);
            return false;
        }
        ...
        if (ret != 0) {
            SHERPA_ONNX_LOGE("Failed to call axclrtSetDevice(). Return code is: %d", static_cast<int32_t>(ret));
            return false;
        }
    ...
    void InitEngine()
    {
        engine_guard_ = std::make_unique<AxclEngineGuard>();
    }
    ...
    void InitContext()
    {
        ...
        if (ret != 0) {
            SHERPA_ONNX_LOGE("Failed to call axclrtEngineCreateContext(). Return code is: %d",
                             static_cast<int32_t>(ret));
            SHERPA_ONNX_EXIT(-1);
        }
    }
    ...
    void InitInput()
    {
        ...
        for (int32_t i = 0; i < num_inputs; ++i) {
            size_t size_in_bytes = axclrtEngineGetInputSizeByIndex(*io_info_guard_, group, i);
            ...
            if (ret != 0) {
                SHERPA_ONNX_LOGE("Failed to call axclrtEngineGetInputDims(). Return code is: %d",
                                 static_cast<int32_t>(ret));
                SHERPA_ONNX_EXIT(-1);
            }
            ...
            input_tensor_names_[i] = axclrtEngineGetInputNameByIndex(*io_info_guard_, i);

            ret = axclrtEngineSetInputBufferByIndex(*engine_io_guard_, i, input_tensors_[i], size_in_bytes);
            if (ret != 0) {
                SHERPA_ONNX_LOGE(
    ...
    void InitOutput()
    {
        ...
        for (int32_t i = 0; i < num_outputs; ++i) {
            auto size_in_bytes = axclrtEngineGetOutputSizeByIndex(*io_info_guard_, group, i);
            ...
            if (ret != 0) {
                SHERPA_ONNX_LOGE("Failed to call axclrtEngineGetOutputDims(). Return code is: %d",
                                 static_cast<int32_t>(ret));
                SHERPA_ONNX_EXIT(-1);
            }

            output_tensor_shapes_.emplace_back(dims.dims, dims.dims + dims.dimCount);
            output_tensor_names_[i] = axclrtEngineGetOutputNameByIndex(*io_info_guard_, i);

            ret = axclrtEngineSetOutputBufferByIndex(*engine_io_guard_, i, output_tensors_[i], size_in_bytes);
            if (ret != 0) {
                SHERPA_ONNX_LOGE(
    ...
    void EnsureThreadContext() const
    {
        ...
        if (ret != 0 || !ctx) {
            SHERPA_ONNX_LOGE("axclrtGetDefaultContext(%d) failed. ret=%d", device_id_rt_, (int)ret);
            SHERPA_ONNX_EXIT(-1);
        }
...
AxclModel::AxclModel(const void *cpu_buf, size_t buf_len_in_bytes, int32_t device_id /*= 0*/)
...
bool AxclModel::SetInputTensorDataRaw(const std::string &name, const void *p, int32_t nbytes) const
{
    return impl_->SetInputTensorDataRaw(name, p, nbytes);
}

std::vector<uint8_t> AxclModel::GetOutputTensorDataRaw(const std::string &name) const
{
...
bool AxclModel::SetInputTensorData(const std::string &name, const float *p, int32_t n) const
{
    return impl_->SetInputTensorData(name, p, n);
}

bool AxclModel::SetInputTensorData(const std::string &name, const int32_t *p, int32_t n) const
{
    return impl_->SetInputTensorData(name, p, n);
}

std::vector<float> AxclModel::GetOutputTensorData(const std::string &name) const
{
    return impl_->GetOutputTensorData(name);
}

bool AxclModel::Run() const
{
    return impl_->Run();
}

bool AxclModel::IsInitialized() const
{
