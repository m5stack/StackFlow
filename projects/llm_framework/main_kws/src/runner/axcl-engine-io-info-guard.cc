...
AxclEngineIOInfoGuard::AxclEngineIOInfoGuard(uint64_t model_id)
{
    ...
    if (ret != 0) {
        SHERPA_ONNX_LOGE("Failed to call axclrtEngineGetIOInfo(). Return code is: %d", static_cast<int32_t>(ret));
        SHERPA_ONNX_EXIT(-1);
    }
...
AxclEngineIOInfoGuard::~AxclEngineIOInfoGuard()
{
    if (initialized_) {
        ...
        if (ret != 0) {
            SHERPA_ONNX_LOGE("Failed to call axclrtEngineDestroyIOInfo(). Return code is: %d",
