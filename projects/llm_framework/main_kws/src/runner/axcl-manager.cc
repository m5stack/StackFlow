...
AxclManager::AxclManager(const char *config /*= nullptr*/)
{
    ...
    if (count_ == 0) {
        ...
        if (ret != 0) {
            SHERPA_ONNX_LOGE("Failed to call axclInit(). Return code: %d", static_cast<int32_t>(ret));
            SHERPA_ONNX_EXIT(-1);
        }
    }
...
AxclManager::~AxclManager()
{
    ...
    if (--count_ == 0) {
        ...
        if (ret != 0) {
            SHERPA_ONNX_LOGE("Failed to call axclFinalize(). Return code: %d", static_cast<int32_t>(ret));
            SHERPA_ONNX_EXIT(-1);
        }
    }
}
