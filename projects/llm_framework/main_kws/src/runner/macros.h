...
#define SHERPA_ONNX_LOGE(...)                                                                \
    do {                                                                                     \
        fprintf(stderr, "%s:%s:%d ", __FILE__, __func__, static_cast<int32_t>(__LINE__));    \
        fprintf(stderr, ##__VA_ARGS__);                                                      \
        fprintf(stderr, "\n");                                                               \
        __android_log_print(ANDROID_LOG_WARN, "sherpa-onnx", "%s:%s:%d", __FILE__, __func__, \
...
#define SHERPA_ONNX_LOGE(...)                                                         \
    do {                                                                              \
        fprintf(stdout, "%s:%s:%d ", __FILE__, __func__, static_cast<int>(__LINE__)); \
...
#define SHERPA_ONNX_LOGE(...)                                                         \
    do {                                                                              \
        fprintf(stderr, "%s:%s:%d ", __FILE__, __func__, static_cast<int>(__LINE__)); \
        fprintf(stderr, ##__VA_ARGS__);                                               \
        fprintf(stderr, "\n");                                                        \
    } while (0)
#endif
...
#define SHERPA_ONNX_READ_META_DATA_VEC_STRING(dst, src_key)                                         \
    do {                                                                                            \
        ...
        SplitStringToVector(value.c_str(), ",", false, &dst);                                       \
                                                                                                    \
        if (dst.empty()) {                                                                          \
            SHERPA_ONNX_LOGE("Invalid value '%s' for '%s'. Empty vector!", value.c_str(), src_key); \
            SHERPA_ONNX_EXIT(-1);                                                                   \
        }                                                                                           \
    } while (0)
...
#define SHERPA_ONNX_READ_META_DATA_STR_WITH_DEFAULT(dst, src_key, default_value) \
    do {                                                                         \
        auto value = LookupCustomModelMetaData(meta_data, src_key, allocator);   \
        if (value.empty()) {                                                     \
            dst = default_value;                                                 \
