...
namespace wetext {
Processor::Processor(const std::string& tagger_path, const std::string& verbalizer_path)
{
    ...
    if (tagger_path.find("zh_tn_") != tagger_path.npos) {
        ...
        parse_type_ = ParseType::kZH_TN;  // 如果是日语的文件开始，也使用中文的规则进行转换
    } else {
        LOG(FATAL) << "Invalid fst prefix, prefix should contain" << " either \"_tn_\" or \"_itn_\".";
    }
}
