CONFIG_TOOLCHAIN_PATH="/opt/gcc-arm-10.3-2021.07-x86_64-aarch64-none-linux-gnu/bin"
CONFIG_TOOLCHAIN_PREFIX="aarch64-none-linux-gnu-"

# CONFIG_COMMPILE_DEBUG=y
CONFIG_AX_MSP_ENABLED=y
CONFIG_AX_650C_MSP_ENABLED=y
CONFIG_MSP_V3_6_2_ENABLED=y
CONFIG_AX630C_OPENWRT_SDK_ENABLED=y
CONFIG_DEVICE_DRIVER_ENABLED=y
CONFIG_DEVICE_UART_ENABLED=y
# CONFIG_LHV_ENABLED=y
# CONFIG_LHV_WITH_KCP is not set
# CONFIG_LHV_WITH_PROTOCOL is not set
# CONFIG_LHV_WITH_EVPP=y
CONFIG_UTILITIES_ENABLED=y
CONFIG_BACKWARD_CPP_ENABLED=y
CONFIG_TOOLCHAIN_FLAGS="-O0"
CONFIG_EVENTPP_ENABLED=y
CONFIG_LHV_ENABLED=y
CONFIG_LHV_WITH_EVPP=y
CONFIG_STACKFLOW_ENABLED=y
CONFIG_UTILITIES_BASE64_ENABLED=y
CONFIG_SINGLE_HEADER_LIBS_ENABLED=y
CONFIG_AX_OPENAI_API_ENABLED=y
CONFIG_AX_SAMPLES_ENABLED=y
CONFIG_AX_SIMPLECV_ENABLED=y
CONFIG_AX_TOKENIZER_ENABLED=y
CONFIG_SIMDJSON_COMPENENT_ENABLED=y
CONFIG_SAMPLE_COMMON_ENABLED=y
CONFIG_RASPBERRYPI_ENV=y
CONFIG_AXCL_ENABLED=y
CONFIG_AXCL_RUNTIME_API_ENABLE=y
CONFIG_AXCL_NATIVE_API_ENABLE=y
CONFIG_AXCL_HAS_STD_FILESYSTEM=y
CONFIG_AXCL_HAS_POSIX_FILE_STAT=y
CONFIG_SHERPA_ONNX_ENABLED=y
CONFIG_SHERPA_NCNN_ENABLED=y
CONFIG_EIGEN_ENABLED=y
CONFIG_KALDI_DECODER_ENABLED=y
CONFIG_KALDIFST_ENABLED=y
