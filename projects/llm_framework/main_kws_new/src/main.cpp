...
public:
            ...
            if (score > threshold) {
                ...
                if (count_frames >= min_continuous_frames) {
                    ...
                    if (trigger_time_ms - last_trigger_time_ms >= REFRACTORY_TIME_MS) {
                        std::cout << "[唤醒触发] 时间: " << trigger_time_ms << " ms\n";
    ...
    kaldi::Matrix<float> compute_fbank(const std::vector<float> &audio16k)
    {
        kaldi::FbankOptions fbank_opts;
        fbank_opts.mel_opts.num_bins     = feat_dim;
        fbank_opts.frame_opts.samp_freq  = resample_rate;
        fbank_opts.frame_opts.dither     = 0.0;
        fbank_opts.energy_floor          = 0.0;
        fbank_opts.mel_opts.htk_mode     = false;
        fbank_opts.use_energy            = false;
        fbank_opts.frame_opts.snip_edges = false;

        kaldi::Fbank fbank(fbank_opts);

        kaldi::Vector<float> wav(audio16k.size());
        for (size_t i = 0; i < audio16k.size(); i++) {
            wav(i) = audio16k[i] * (1 << 15);
        }

        kaldi::Matrix<float> features;
        fbank.ComputeFeatures(wav, resample_rate, 1.0, &features);
        return features;  // Shape: [num_frames, feat_dim]
    }

    // std::vector<float> run_inference(const std::vector<float> &audio16k)
    // {
    //     auto feats = compute_fbank(audio16k);

    //     std::array<int64_t, 3> input_shape{1, feats.NumRows(), feats.NumCols()};
    //     Ort::Value input_tensor = Ort::Value::CreateTensor<float>(
    //         memInfo, (float *)feats.Data(), feats.NumRows() * feats.NumCols(), input_shape.data(),
    //         input_shape.size());

    //     // cache 作为输入
    //     std::array<int64_t, 4> cache_shape{1, 32, 88, 1};
    //     Ort::Value cache_tensor = Ort::Value::CreateTensor<float>(
    //         memInfo, (float *)cache.Data(), cache.NumRows() * cache.NumCols(), cache_shape.data(),
    //         cache_shape.size());

    //     const char *input_names[]  = {"input", "cache"};
    //     const char *output_names[] = {"output", "cache_out"};

    //     auto outputs = ort_session->Run(Ort::RunOptions{nullptr}, input_names, &input_tensor,
    //                                     1,  // TODO: pass cache_tensor too (update input count)
    //                                     output_names, 2);

    //     auto &out_chunk_tensor = outputs[0];
    //     float *out_data        = out_chunk_tensor.GetTensorMutableData<float>();
    //     size_t out_len         = out_chunk_tensor.GetTensorTypeAndShapeInfo().GetElementCount();

    //     return std::vector<float>(out_data, out_data + out_len);
    // }
